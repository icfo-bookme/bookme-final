...
const TourSearchBar = ({ data }) => {
  ...
  // Typewriter effect for placeholder with "Search Tours For" prefix
  useEffect(() => {
    if (!data?.length) return;

    const typewriterItems = data.map(dest => `${dest.name}, ${dest.country}`);
    const prefix = "Search Tours For ";
        ...
        <div className="gird grid-cols-2">
          <div className="grid grid-cols-1 gap-4 relative" ref={searchRef}>
            <div className="space-y-1 relative">
              <label className="block text-sm text-blue-950 font-medium">TOUR DESTINATION</label>
